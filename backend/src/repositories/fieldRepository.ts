...
export async function createField(input: {
  ...
  const result = await pool.query<FieldRow>(
    `INSERT INTO fields (name, crop_type, planting_date, current_stage)
     VALUES ($1, $2, $3, $4)
     RETURNING id, name, crop_type, planting_date, current_stage, assigned_agent_id, NULL::text AS assigned_agent_name`,
    [input.name, input.cropType, input.plantingDate, input.currentStage]
  );

  return result.rows[0];
}

export async function updateField(id: string, input: Partial<{ name: string; cropType: string; plantingDate: string; currentStage: CropStage }>): Promise<FieldRow | null> {
  const existing = await pool.query(
    'SELECT id, name, crop_type, planting_date, current_stage FROM fields WHERE id = $1',
    [id]
  );
  if (!existing.rows[0]) {
    return null;
  ...
  const result = await pool.query<FieldRow>(
    `UPDATE fields
     ...
     WHERE id = $1
     RETURNING id, name, crop_type, planting_date, current_stage, assigned_agent_id, NULL::text AS assigned_agent_name`,
    [id, updated.name, updated.cropType, updated.plantingDate, updated.currentStage]
  );
...
export async function assignField(fieldId: string, agentId: string): Promise<FieldRow | null> {
  const result = await pool.query<FieldRow>(
    `UPDATE fields f
     ...
     RETURNING f.id, f.name, f.crop_type, f.planting_date, f.current_stage, f.assigned_agent_id, u.name AS assigned_agent_name`,
    [fieldId, agentId]
  );
  return result.rows[0] ?? null;
}
